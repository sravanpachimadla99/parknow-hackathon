...
class User(Base):
    ...
    vtype = Column(Enum('car', 'bike', 'suv'), default='car', nullable=False)
...
class Slot(Base):
    ...
    type = Column(Enum('car', 'bike', 'suv'), nullable=False)
    status = Column(Enum('free', 'occupied', 'reserved'), default='free', nullable=False, index=True)
...
class Booking(Base):
    ...
    zone_name = Column(String(60), nullable=False)
    vehicle = Column(String(20), nullable=False)
    vtype = Column(Enum('car', 'bike', 'suv'), nullable=False)
    date = Column(Date, nullable=False, index=True)
    time = Column(Time, nullable=False)
    end_time = Column(Time, nullable=False)
    dur = Column(SmallInteger, nullable=False)
    cost = Column(SmallInteger, nullable=False)
    pay = Column(Enum('upi', 'card', 'wallet', 'netbanking', 'qr'), nullable=False)
    status = Column(Enum('active', 'cancelled', 'completed'), default='active', nullable=False)
    payment_status = Column(Enum('pending', 'paid', 'failed'), default='pending', nullable=False)
